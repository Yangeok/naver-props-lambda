...
export const MapMenu: React.FC<IMapMenu> = ({ mapTypeIds, setMapTypeIds }) => {
  ...
  return (
    <div ref={dropdownRef} className="absolute z-10 h-10 bg-no-repeat w-50 top-1 left-[50px]">
      <button
      ...
      {isDropdownOpen && <form onSubmit={handleSubmit(onSubmit)} className="absolute z-50 top-10">
        <Controller
          name="selectedTypes"
          control={control}
          render={({ field }) => (
